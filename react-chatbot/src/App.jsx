...
function App() {
    ...
    const retrieveDocument = async () => {
        try {
            const response = await axios.get('api/v1/sharepoint/read-files-from-folder', {
    ...
    const handleLeaveBalanceCheck = async (employeeId) => {
      try {
          // Fetch leave balances from the API
          const response = await axios.get(`http://localhost:3001/api/leave-requests/leave-balance/66eaf141b2d2c3c0b2da7878`);
          const leaveBalances = response.data;
  
          // Log the fetched leave balances and a confirmation message
          console.log('Fetched leave balances:', leaveBalances);
        
  
          // Check if leaveBalance exists and has values
          if (leaveBalances.leaveBalance && Object.keys(leaveBalances.leaveBalance).length > 0) {
              // Create messages for each leave balance
              const balanceMessages = Object.entries(leaveBalances.leaveBalance).map(([leaveType, balance]) => ({
                  text: `Leave Type: ${leaveType}, Available Balance: ${balance}`,
                  sender: 'bot',
              }));
  
              // Update messages in one go
              setMessages((prevMessages) => [
                  ...prevMessages,
                  { text: `Checking your leave`, sender: 'bot' },
                  ...balanceMessages,
              ]);
          } else {
              // Handle the case where there are no leave balances
              const noBalanceMessage = 'No leave balances left for this Employee ID.';
              console.log(noBalanceMessage); // Log the message to the console
              setMessages((prevMessages) => [
                  ...prevMessages,
                  { text: noBalanceMessage, sender: 'bot' },
              ]);
          }
      } catch (error) {
          ...
          setMessages((prevMessages) => [
              ...prevMessages,
              { text: 'Error retrieving leave balances. Please try again.', sender: 'bot' },
          ]);
      }
  };
